namespace antlr
{
	...
	/// product incorrect token index values.  That is, an insert operation at token
	...
	/// ...
	/// rewriteEngine.insertAfter("pass1", t, "func1");}
	...
	internal class TokenStreamRewriteEngine : TokenStream
	{
		...
		/// </summary>
		protected int _index = MIN_TOKEN_INDEX;
		...
		public IToken nextToken() 				// throws TokenStreamException 
		{
			...
			do 
			{
				...
				if ( t != null ) 
				{
					t.setIndex(_index);  // what is t's index in list?
					if ( t.Type != Token.EOF_TYPE ) 
					{
						tokens.Add(t);  // track all tokens except EOF
					}
					_index++;			// move to next position
				}
