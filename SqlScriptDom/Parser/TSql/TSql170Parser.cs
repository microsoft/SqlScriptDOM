//------------------------------------------------------------------------------
// <copyright file="TSql170Parser.cs" company="Microsoft">
...
{
    /// <summary>
    /// The TSql Parser for 17.0.
    /// </summary>
    [Serializable]
    public class TSql170Parser : TSqlParser
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TSql170Parser"/> class.
        /// </summary>
        /// <param name="initialQuotedIdentifiers">if set to <c>true</c> [initial quoted identifiers].</param>
        public TSql170Parser(bool initialQuotedIdentifiers)
        ...
        public TSql170Parser(bool initialQuotedIdentifiers, SqlEngineType engineType)
            : base(initialQuotedIdentifiers)
        {
        ...
        {
            return new TSql170LexerInternal();
        }

        #region Some utility stuff
        TSql170ParserInternal GetNewInternalParser()
        {
            return new TSql170ParserInternal(QuotedIdentifier);
        }

        TSql170ParserInternal GetNewInternalParserForInput(TextReader input, out IList<ParseError> errors, 
            int startOffset, int startLine, int startColumn)
        {
            TSql170ParserInternal parser = GetNewInternalParser();
        ...
        public override TSqlFragment Parse(IList<TSqlParserToken> tokens, out IList<ParseError> errors)
        {
            ...
            if (result != null)
            {
                VersioningVisitor versioningVisitor = new VersioningVisitor(engineType, SqlVersion.Sql170);
        ...
        private void HotswapCreateExternalStreamingJobStatements(TSqlFragment fragment)
        {
            ...
            foreach (TSqlBatch batch in ((TSqlScript)fragment).Batches)
            {
                ...
                if (proc?.Parameters == null || proc.Parameters.Count < 2)
                {
                    continue;
                }
        ...
        {
            if (statement?.ExecuteSpecification == null)
            {
                return null;
            }
        ...
        {
            TSql170ParserInternal parser = GetNewInternalParserForInput(input, out errors, startOffset, startLine, startColumn);
