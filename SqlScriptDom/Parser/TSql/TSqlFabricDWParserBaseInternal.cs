//------------------------------------------------------------------------------
// <copyright file="TSqlFabricDWParserBaseInternals.cs" company="Microsoft">
...
{
    internal abstract class TSqlFabricDWParserBaseInternal : TSql150ParserBaseInternal
    {
        #region Constructors

        // Not really needed, here only because ANTLR generates call to this one in derived classes
        protected TSqlFabricDWParserBaseInternal(TokenBuffer tokenBuf, int k)
            : base(tokenBuf, k)
        {
        }

        // Not really needed, here only because ANTLR generates call to this one in derived classes
        protected TSqlFabricDWParserBaseInternal(ParserSharedInputState state, int k)
            : base(state, k)
        {
        }

        // Not really needed, here only because ANTLR generates call to this one in derived classes
        protected TSqlFabricDWParserBaseInternal(TokenStream lexer, int k)
            : base(lexer, k)
        {
        }
        ...
        public TSqlFabricDWParserBaseInternal(bool initialQuotedIdentifiersOn)
        ...
        /// <summary>
        /// Verifies the option is valid for SQL FabricDW platform
        /// </summary>
        protected static void VerifyAllowedIndexOptionFabricDW(IndexAffectingStatement statement, IndexOption option)
        {
        ...
        protected static SqlDataTypeOption ParseDataTypeFabricDW(string token)
        {
        ...
        /// <param name="isLedgerSupported">Ledger is only supported for version SqlFabricDW or above (True for SqlFabricDW version)</param>
